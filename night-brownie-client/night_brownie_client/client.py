"""HTTP client for the Night Brownie agent harness."""
...
class NightBrownieClientError(Exception):
    """Raised when the Night Brownie harness returns a non-2xx HTTP response.
...
class NightBrownieClient:
    """Synchronous HTTP client for the Night Brownie agent harness.
    ...
    Args:
        harness_url: Base URL of the running Night Brownie harness (e.g. `"http://localhost:8000"`).
        agent_url: This agent's own base URL, sent when claiming tasks so
            the harness knows which agent holds the claim.
        timeout: HTTP request timeout in seconds applied to all requests. Defaults to `5.0`.

    Example:
        >>> with NightBrownieClient(
    ...
    def __init__(self, harness_url: str, agent_url: str, timeout: float = 5.0) -> None:
        ...
        self._http = httpxyz.Client(base_url=harness_url, timeout=timeout)
    ...
    def next_task(self) -> TaskMessage | None:
        ...
        Returns:
            A `night_brownie_client.models.TaskMessage` when a task is
        ...
        raise NightBrownieClientError(response.status_code, response.text)
    ...
    def complete_task(self, task_id: str, decision: DecisionMessage) -> None:
        ...
        Args:
            task_id: The `task_id` from the original
                `night_brownie_client.models.TaskMessage`.
            decision: The agent's `night_brownie_client.models.DecisionMessage`
                to store.
        """
        ...
        if not complete_resp.is_success:
            ...
            raise NightBrownieClientError(complete_resp.status_code, complete_resp.text)
        ...
        if not nudge_resp.is_success:
            ...
            raise NightBrownieClientError(nudge_resp.status_code, nudge_resp.text)
